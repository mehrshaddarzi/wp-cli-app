...
$GLOBALS['WP_CLI_APP'] = array(
	 ...
	 */
	'cache_dir'                  => WP_CLI_APP_CACHE_PATH,
	/**
	 * Online document url
	 */
	'docs'                       => 'https://docs.wp-cli-app.com/',
	/**
	 ...
	 * Commands log
	 */
	'command_log'                => WP_CLI_APP_CACHE_PATH . '/log.json',
	...
	'package'                    => array(
		...
		'version'                  => array(
			'file' => WP_CLI_APP_CACHE_PATH . '/version.json',
			'age'  => 3, //Hour
		),

		# Save List Of Wordpress Locale in cache
		'locale'                   => array(
			'file' => WP_CLI_APP_CACHE_PATH . '/locale.json',
			'age'  => 168, //168 Hour = One Week
		),

		# Package LocalTemp
		'localTemp'                => array(
			'path' => WP_CLI_APP_HOME_PATH . '/package-temp',
			'age'  => 8765, //1 year
			'type' => '.json'
		),

		# Save WordPress Download Url List
		'wordpress_core_url_file'  => WP_CLI_APP_CACHE_PATH . '/core-url.json',
	...
	/* List Format */
	'list_format_dir'            => '_list',

	/* acl */
	'acl_opt'                    => 'wp_cli_acl',

	/* Workspace */
	//Options name for current Workspace in wp_options table
	'workspace'                  => 'wp-cli-app-workspace',
	'config_file'                => 'cli.json',

	/* plugin */
	'default_plugin_template'    => 'app',
	'default_plugin_name'        => 'A new wordpress Plugin',
	'default_plugin_slug'        => 'my-new-plugin',
	'default_plugin_description' => 'Basic WordPress Plugin Header Comment',
	'default_plugin_url'         => 'https://wordpress.org',
	'default_author_name'        => 'wordpress.org',
	'default_author_url'         => 'https://author.example.com/',
	'default_text_domain'        => 'plugin-text-domain',

	/* Ftp */
	'plugin_file_name'           => 'wp_cli_ftp.php',
	'ftp_opt_file_name'          => WP_CLI_APP_CACHE_PATH . '/remote.json',
...
/**
 * Get WP-CLI APP config
 *
 * @return mixed
 */
function get_wp_cli_app_config() {
	$list    = $GLOBALS['WP_CLI_APP'];
...
/**
 * Loader commands in wordpress cli application
 *
 * @param $slug
 * @param boolean $separator
 * @throws Exception
 */
function WP_CLI_APP_LOADER_COMMAND( $slug, $separator = false ) {
	//Get all files in command dir
	foreach ( glob( rtrim( str_replace( "\\", "/", __DIR__ ), "/" ) . "/command/" . $slug . "/*.php" ) as $filename ) {
		...
		\WP_CLI::add_command( $GLOBALS['WP_CLI_APP']['command'] . ' ' . ( $separator === false ? '' : $slug . $separator ) . $command, "wp_cli_app_" . $slug . "_" . str_replace( "-", "_", $command ) );
	}
}