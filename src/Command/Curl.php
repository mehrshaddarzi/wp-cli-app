...
class Curl {
	...
	public function __construct( $url, $where = '', $new_name = '' ) {
		...
		$this->cwd      = PHP::getcwd();
		$downloads_dir  = FileSystem::path_join( $this->cwd, $GLOBALS['WP_CLI_APP']['downloads_dir'] );
	...
	public function download( $options = array() ) {
		...
		if ( isset( $options['user-agent'] ) ) {
			curl_setopt( $curl, CURLOPT_USERAGENT, get_wp_cli_app_config( 'curl', 'user_agent' ) );
		}
		curl_exec( $curl );
		if ( curl_error( $curl ) ) {
