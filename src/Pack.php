...
use WP_CLI_APP\Package\Utility\help;
use WP_CLI_APP\Package\Utility\update;
use WP_CLI_APP\Package\Utility\validation;
use WP_CLI_APP\Package\Utility\view;
 ...
 * ## DOCUMENT
 *
 *      https://docs.wp-cli-app.com/WordPress-Package/
 *
 * @package wp-cli
 */
class Pack extends \WP_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *      # Show WP-CLI application Documentation in the web browser.
	 *      $ wp app docs
	 *
	 *      # Show '/config' page from WP-CLI application Documentation in the web browser.
	 *      $ wp app docs config
	 *
	 * @when before_wp_load
	 * @alias doc
	...
	function docs( $_, $assoc ) {
		...
		//Get basic docs url
		$url = get_wp_cli_app_config( 'docs' );
		...
		if ( $web_url === false ) {
			$web_url = get_wp_cli_app_config( 'docs' );
		}
