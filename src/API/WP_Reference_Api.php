...
class WP_Reference_Api {
	...
	public function __construct() {
		...
		if ( ! class_exists( '\DOMDocument' ) ) {
			CLI::error( $this->log['DOMDocument'] );
		}
		/*
		 * Set Default Variable
		 */
		$this->path                  = FileSystem::path_join( CLI::get_cache_dir(), $this->dir );
		$this->last_search_file_path = FileSystem::path_join( $this->path, $this->last_search_file );
	}
	...
	public function run_clear_cache() {
		//Confirm Cli
		CLI::confirm( $this->log['confirm_remove_cache'] );

		//Remove Folder
		if ( FileSystem::folder_exist( $this->path ) ) {
			FileSystem::remove_dir( $this->path, false );
		}

		CLI::success( $this->log['remove_cache'] );
	}
		...
		if ( $LocalCache != false ) {
			...
			if ( $response['status'] == "redirect" ) {
				...
				if ( @$dom->loadHTML( $response['content'] ) === false ) {
					CLI::pl_wait_end();
					CLI::error( $this->log['dom'] );
				}
				if ( $this->is_404_page( $dom ) === true ) {
					CLI::pl_wait_end();
					CLI::error( $this->log['not_found'] );
					exit;
				}
	...
	private function sanitize_search_word( $search ) {
		...
		if ( trim( $word ) == "" ) {
			...
			CLI::pl_wait_end();
			CLI::error( $this->log['enter_search_keyword'] );
			exit;
		} else {
			return $search;
		}
	...
	private function end_command_process( $content, $last_link_data, $options = array() ) {
		...
		if ( isset( $last_link_data ) ) {
			FileSystem::create_json_file( $this->last_search_file_path, $last_link_data, false );
		}
	...
	private function show_result_command( $content, $options = array() ) {
		...
		if ( isset( $content['structure'] ) ) {
			...
			//Structure
			if ( isset( $content['structure'] ) ) {
				CLI::line( CLI::color( "# Structure", $title_color ) );
				CLI::br();
				...
				for ( $i = 0; $i < count( $exp ); $i ++ ) {
					if ( $i == 0 ) {
						...
						$word    .= CLI::color( $explode[0] . "( ", "Y" ) . CLI::color( $explode[1], "P" );
					} else {
						if ( stristr( $exp[ $i ], "," ) ) {
							$explode = explode( ", ", $exp[ $i ] );
							$word    .= CLI::color( $explode[0] . ", ", "Y" ) . CLI::color( $explode[1], "P" );
						} else {
							$word .= CLI::color( $exp[ $i ], "Y" );
						}
					}
					$word .= " ";
				}
				...
				if ( isset( $content['summary'] ) and ! empty( $content['summary'] ) ) {
					CLI::line( "Summary   : " . $content['summary'] );
				}

				//Show Type
				if ( isset( $content['type'] ) and ! empty( $content['type'] ) ) {
					CLI::line( "Reference : " . $content['type'] );
				}

				//show Url
				if ( isset( $content['page_url'] ) and ! empty( $content['page_url'] ) ) {
					CLI::line( "Url       : " . $content['page_url'] );
				}

				//Source File
				if ( isset( $content['source'] ) and ! empty( $content['source']['file'] ) ) {
					CLI::line( "Source    : " . trim( $content['source']['file'] ) . ( isset( $content['source']['line'] ) ? ":" . trim( $content['source']['line'] ) : '' ) );
				}
			...
			if ( isset( $content['description'] ) and ! empty( $content['description'] ) ) {
				CLI::br();
				CLI::line( CLI::color( "# Description", $title_color ) );
				CLI::br();
				CLI::log( " " . $content['description'] );
			}

			//Show Parameter
			if ( isset( $content['parameters'] ) and ! empty( $content['parameters'] ) ) {
				CLI::br();
				CLI::line( CLI::color( "# Parameters", $title_color ) );
				CLI::br();

				foreach ( $content['parameters'] as $k_param => $v_param ) {
					//Show Param name
					CLI::line( " " . CLI::color( $k_param, "Y" ) );

					//Show Param Option
					CLI::line( "  " . CLI::color( "(" . $v_param['type'] . ")", "P" ) . " " . CLI::color( "{" . $v_param['required'] . "}", "B" ) );

					//Show Desc
					CLI::line( "    " . $v_param['description'] );
					...
					if ( isset( $v_param['param'] ) and ! empty( $v_param['param'] ) ) {
						...
						foreach ( $v_param['param'] as $_param_k => $_param_v ) {
							CLI::line( "       " . $_param_k );
							CLI::line( "         " . "(" . $_param_v['type'] . ")" );
							CLI::line( "            " . "(" . $_param_v['desc'] . ")" );
							CLI::br();
						}
					}
				...
				if ( isset( $content['return'] ) and ! empty( $content['return'] ) ) {
					CLI::br();
					CLI::line( CLI::color( "# Return", $title_color ) );
					CLI::br();
					CLI::line( " (" . trim( $content['return']['type'] ) . ") " . trim( $content['return']['desc'] ) );
				}

				//Show Change Log
				if ( isset( $content['changelog']['tbody'] ) and ! empty( $content['changelog']['tbody'] ) ) {
					CLI::br();
					CLI::line( CLI::color( "# Changelog", $title_color ) );
					CLI::br();
					CLI::create_table( $content['changelog']['tbody'] );
				}
			...
			foreach ( $content as $_s_key => $_s_value ) {
				CLI::line( "{$_s_key}. " . CLI::color( $_s_value['title'], "Y" ) . " " . CLI::color( "[" . ucfirst( $_s_value['type'] ) . "]", "B" ) );
				CLI::line( CLI::color( "     Source: " . $_s_value['file'] . ":" . $_s_value['line'], "P" ) );
				CLI::line( "      " . $_s_value['desc'] );
				CLI::br();
			}
			CLI::br();
	...
	private function exist_link_ID( $ID, $log = true ) {
		...
		if ( $json != false and is_array( $json ) and array_key_exists( $ID, $json ) ) {
			...
			if ( $log ) {
				CLI::error( $this->log['not_found_id'] );
			} else {
				return false;
			}
		}
	...
	private function read_last_link_file( $log = true ) {
		...
		if ( ! file_exists( $this->last_search_file_path ) ) {
			if ( $log ) {
				CLI::error( $this->log['empty_search_history'] );
			} else {
				return false;
			}
		}
		...
		//Read File
		$json = FileSystem::read_json_file( $this->last_search_file_path );
		if ( $json === false ) {
			FileSystem::remove_file( $this->last_search_file_path );
			if ( $log ) {
				CLI::error( $this->log['empty_search_history'] );
			} else {
				return false;
	...
	private function create_cache_dir() {
		if ( ! FileSystem::folder_exist( $this->path ) ) {
			FileSystem::create_dir( $this->dir, CLI::get_cache_dir() );
		}
	}
	...
	private function check_cache_url( $url ) {
		...
		//Search in files
		$file_path = FileSystem::path_join( $this->path, $file_name . ".json" );
		if ( file_exists( $file_path ) ) {
			return FileSystem::read_json_file( $file_path );
		}
	...
	private function create_cache_data( $url, $data ) {
		...
		$file_path = FileSystem::path_join( $this->path, $file_name . ".json" );
		if ( FileSystem::create_json_file( $file_path, $data, false ) ) {
			return true;
		}
	...
	private function remove_dynamic_cache_file() {
		...
		$list_of_cache_file = FileSystem::sort_dir_by_date( $this->path, "ASC" );
		...
		if ( $number_file > $this->max_cache_reference ) {
			...
			for ( $i = 0; $i < $_to_remove; $i ++ ) {
				FileSystem::remove_file( FileSystem::path_join( $this->path, $list_of_cache_file[ $i ] ) );
			}
		}
	}
	...
	private static function remove_whitespace_word( $content ) {
		return PHP::remove_whitespace_word( $content );
	}
