...
class WP_Plugins_Api {
	...
	public function __construct() {
		 ...
		 * Set Default config
		 */
		$this->config    = $GLOBALS['WP_CLI_APP']['wordpress_api']['plugins'];
		$this->cache_dir = rtrim( WP_CLI_APP_CACHE_PATH, "/" ) . '/' . $this->config['cache_dir'];
		/*
		 * Check Cache Dir
		 */
		if ( FileSystem::folder_exist( $this->cache_dir ) === false ) {
			FileSystem::create_dir( $this->config['cache_dir'], WP_CLI_APP_CACHE_PATH );
		}
	}
