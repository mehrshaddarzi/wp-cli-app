...
class FTP {
	...
	public function __construct() {
		...
		//Set Config File
		$this->config_file   = $GLOBALS['WP_CLI_APP']['ftp_opt_file_name'];
		$this->template_path = FileSystem::path_join( WP_CLI_APP_PATH, 'templates/' . $GLOBALS['WP_CLI_APP']['plugin_file_name'] );
	...
	public function check_ftp_domain( $domain = '', $base_path = '' ) {
		...
		$data     = array(
			'GET_KEY'   => $get_key,
			'file_name' => $GLOBALS['WP_CLI_APP']['plugin_file_name'],
		);
		...
		//Upload To Server
		try {
			$remote_file_path = FileSystem::path_join( $wp_path, $GLOBALS['WP_CLI_APP']['plugin_file_name'] );
			$this->ftp->put( $remote_file_path, $this->template_path );
			@FileSystem::remove_file( $this->template_path );

			//Connect To Host
			$request = CLI::http_request( rtrim( $url, "/" ) . "/" . $GLOBALS['WP_CLI_APP']['plugin_file_name'] . '?wp_cli_ftp_token=' . $get_key );
			if ( $request === false ) {
	...
	public function json_request( $key ) {
		$request = CLI::http_request( rtrim( $this->domain, "/" ) . "/" . $GLOBALS['WP_CLI_APP']['plugin_file_name'] . '?wp_cli_ftp_token=' . $key );
		if ( $request === false ) {
			@$this->ftp->delete( FileSystem::path_join( $this->wp_directory, $GLOBALS['WP_CLI_APP']['plugin_file_name'] ) );
			return false;
		} else {
			$json = json_decode( $request, true );
			return $json;
	...
	public function prepare_plugin( $template, $remote_path = '/', $args = array() ) {
		...
		try {
			...
			$this->ftp->put( FileSystem::path_join( $remote_path, $GLOBALS['WP_CLI_APP']['plugin_file_name'] ), $this->template_path );
			@FileSystem::remove_file( $this->template_path );
			return $get_key;
		} catch ( \Exception $e ) {
			@FileSystem::remove_file( $this->template_path );
