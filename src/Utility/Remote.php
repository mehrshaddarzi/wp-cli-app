...
class Remote {
	...
	public function __construct() {
		...
		//Check Wp Cli Application is running
		if ( ! isset( $GLOBALS['WP_CLI_APP'] ) ) {
			exit;
		}

		//Prepare app config
		$this->wp_cli_app = $GLOBALS['WP_CLI_APP'];
	...
	public function backup( $args = array() ) {
		...
		if ( $args['where'] == "remove" ) {
			...
			$key = $this->wp_ftp->prepare_plugin( 'backup/backup-remove', $this->config['mu_plugins'], array( 'do_action' => array( 'helper/set-time-limit', 'helper/remove-folder' ), 'backup_dir' => $this->wp_cli_app['backup_dir'] ) );
			if ( $key === false ) {
		...
		if ( $args['where'] == "list" ) {
			...
			//Prepare Plugin and Upload to server
			$key = $this->wp_ftp->prepare_plugin( 'backup/backup-list', $this->config['mu_plugins'], array( 'do_action' => array( 'helper/set-time-limit' ), 'backup_dir' => $this->wp_cli_app['backup_dir'] ) );
			if ( $key === false ) {
				CLI::error( $this->notice['upload_error'] );
				exit;
			}
		...
		$backup_dir = FileSystem::path_join( $this->config['wp_directory'], $this->wp_cli_app['backup_dir'] );
		try {
		...
		$where_arg = array(
			'db'   => array(
				...
				'args'      => array(
					...
					'backup_dir' => $this->wp_cli_app['backup_dir']
				)
			),
			'file' => array(
		...
		if ( array_key_exists( 'download_after', $args ) and isset( $request ) ) {
			CLI::run( 'tools curl ' . $request['link'] . ' ' . $this->wp_cli_app['downloads_dir'], true );
		}
	...
	public function login( $args = array() ) {
		...
		$key = $this->wp_ftp->prepare_plugin( 'user-login', $this->config['mu_plugins'],
			array(
				...
				'wp_acl'         => $this->wp_cli_app['acl_opt'],
