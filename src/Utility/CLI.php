...
class CLI {
	...
	public static function _e( $section, $key, $replace = array() ) {
		...
		$log = null;
		if ( isset( $GLOBALS['WP_CLI_APP'][ $section ]['log'][ $key ] ) ) {
			$log = $GLOBALS['WP_CLI_APP'][ $section ]['log'][ $key ];
		}
	...
	public static function pl_wait_start( $new_line = false, $color = 'B' ) {
		...
		if ( ! defined( 'WP_CLI_APP_PLEASE_WAIT_LOG' ) ) {
			define( 'WP_CLI_APP_PLEASE_WAIT_LOG', true );
		}
	...
	public static function exist_wp_cli_command( $cmd ) {
		...
		$ext     = explode( " ", $cmd );
		$command = array();
		if ( in_array( strtolower( $ext[0] ), $GLOBALS['WP_CLI_APP']['command_prefix'] ) ) {
	...
	/**
	 * get before command which run in wp-cli application
	 *
	 * @return array
	 */
	public static function get_command_log() {
		...
		$file = $GLOBALS['WP_CLI_APP']['command_log'];
		if ( file_exists( $file ) ) {
	...
	public static function lunch_editor( $file_path, $editor = false ) {
		...
		if ( self::is_windows() and $editor != false ) {
			...
			# Lunch with WP-CLI
			$r = \WP_CLI\Utils\launch_editor_for_input( $contents, get_wp_cli_app_config( 'package', 'file' ), 'json' );
			if ( $r === false ) {
				CLI::warning( 'No changes made to ' . get_wp_cli_app_config( 'package', 'file' ) );
			} else {
