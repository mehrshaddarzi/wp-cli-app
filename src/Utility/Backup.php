...
class Backup {
	...
	public function __construct() {
		...
		if ( ! FileSystem::folder_exist( FileSystem::path_join( ABSPATH, $WP_CLI_APP['backup_dir'] ) ) ) {
			FileSystem::create_dir( $WP_CLI_APP['backup_dir'], ABSPATH );
		}

		//Set Default folder
		$this->base_path       = ABSPATH;
		$this->backup_dir      = $WP_CLI_APP['backup_dir'];
		$this->backup_dir_path = FileSystem::path_join( ABSPATH, $WP_CLI_APP['backup_dir'] );
	...
	public function full_backup( $file_name = '' ) {
		...
		FileSystem::create_zip( $this->base_path, FileSystem::path_join( $this->backup_dir_path, $new_name ), "wordpress", array( $this->backup_dir . "/", $GLOBALS['WP_CLI_APP']['list_format_dir'] . "/", $GLOBALS['WP_CLI_APP']['downloads_dir'] . "/", ".idea/" ) );
