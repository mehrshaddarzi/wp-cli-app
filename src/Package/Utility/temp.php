...
class temp {
	...
	public static function get_temp_file_name( $cwd ) {
		return FileSystem::path_join( get_wp_cli_app_config( 'package', 'localTemp', 'path' ), self::convert_path_to_file( $cwd ) . get_wp_cli_app_config( 'package', 'localTemp', 'type' ) );
	}
	...
	public static function clear_temp( $force = false ) {
		$localPath = get_wp_cli_app_config( 'package', 'localTemp', 'path' );
		if ( realpath( $localPath ) ) {
			$list_file = FileSystem::get_dir_contents( $localPath );
			foreach ( $list_file as $file ) {
				$file = $file . get_wp_cli_app_config( 'package', 'localTemp', 'name' );
				if ( $force === true || FileSystem::check_file_age( $file, get_wp_cli_app_config( 'package', 'localTemp', 'age' ) ) ) {
	...
	public static function get_temp( $cwd ) {
		...
		$list      = FileSystem::get_dir_contents( get_wp_cli_app_config( 'package', 'localTemp', 'path' ) );
		foreach ( $list as $file_path ) {
			$file_path = $file_path . get_wp_cli_app_config( 'package', 'localTemp', 'name' );
