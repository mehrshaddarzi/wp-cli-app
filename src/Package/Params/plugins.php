		...
		if ( is_string( $array ) ) {
			...
			if ( PHP::is_assoc_array( $array ) === false ) {
				...
				foreach ( $array as $plugin_slug => $plugin_v ) {
					...
					$slug = PHP::to_lower_string( preg_replace( $GLOBALS['WP_CLI_APP']['wordpress_api']['slug'], '', $plugin_slug ) );
					...
					if ( is_string( $plugin_v ) ) {
						...
						if ( empty( PHP::to_lower_string( $plugin_v ) ) ) {
							...
							if ( $plugin_url === false ) {
								...
								if ( ! in_array( $plugin_v, $GLOBALS['WP_CLI_APP']['package']['latest'] ) ) {
						...
						if ( array_key_exists( 'version', $plugin_v ) and empty( $url ) ) {
							...
							//Check Latest
							if ( ! in_array( $plugin_v['version'], $GLOBALS['WP_CLI_APP']['package']['latest'] ) ) {
				...
				if ( ! $valid->is_cli_error() ) {
					...
					foreach ( $plugin_list as $plugin ) {
						...
						if ( isset( $plugin['url'] ) ) {
							...
							if ( defined( 'WP_CLI_APP_PACKAGE_RUN_EXIST_CUSTOM_URL' ) ) {
