...
use WP_CLI_APP\Package\Arguments\Config as Config_Arg;
use WP_CLI_APP\Package\Arguments\Cookie;
...
class config {
							...
							if ( PHP::to_lower_string( $var ) != PHP::to_lower_string( $raw_title ) ) {
								...
								if ( empty( $parameter['url'] ) ) {
									...
									if ( filter_var( $parameter['url'], FILTER_VALIDATE_URL ) === false ) {
										...
										if ( defined( 'WP_CLI_APP_PACKAGE_RUN_CHECK_SITE_URL' ) ) {
	...
	public function sanitize_theme( $var, $validate = false ) {
		...
		if ( empty( $var ) ) {
			...
			//Check theme slug
			$slug = PHP::to_lower_string( preg_replace( $GLOBALS['WP_CLI_APP']['wordpress_api']['slug'], '', $var ) );
			if ( $slug != $var ) {
