...
use WP_CLI_APP\Package\Arguments\Version;
...
class core {
	...
	public function validation( $pkg_array ) {
		...
		//Default value
		$default_config = $GLOBALS['WP_CLI_APP']['package']['default'];
			...
			if ( is_string( $parameter ) ) {
				...
				if ( PHP::is_assoc_array( $parameter ) === false ) {
					...
					if ( ! $valid->is_cli_error() ) {
						...
						if ( ! $valid->is_cli_error() and \WP_CLI_APP\Package\Arguments\Core::is_installed_wordpress() === false ) {
	...
	public function update( $pkg_array ) {
		...
		\WP_CLI_APP\Package\Arguments\Core::update_network( $pkg_array );
	}
