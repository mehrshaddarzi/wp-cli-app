...
class Config {
	...
	public static function add_wordpress_package_anchor() {
		FileSystem::search_replace_file(
			...
			array(
				"" . $GLOBALS['WP_CLI_APP']['package']['wordpress_package_anchor'] . "\n" . $GLOBALS['WP_CLI_APP']['package']['wp_package_end_anchor'] . "\n\n/* That's all",
	...
	public static function get_line_of_package_anchor() {
		...
		$x      = 1;
		foreach ( $source as $l ) {
			if ( stristr( trim( $l ), $GLOBALS['WP_CLI_APP']['package']['wordpress_package_anchor'] ) != false ) {
				$line['from'] = $x;
			}
			if ( stristr( trim( $l ), $GLOBALS['WP_CLI_APP']['package']['wp_package_end_anchor'] ) != false ) {
				$line['to'] = $x;
			}
			$x ++;
		}
	...
	public static function get_tbl_prefix() {
		...
		return isset( $variable['table_prefix']['value'] ) ? PHP::remove_quote( $variable['table_prefix']['value'] ) : get_wp_cli_app_config( 'package', 'default', 'table_prefix' );
	}
	...
	public static function update_constant( $pkg_constant, $current_const_list = array(), $options = array() ) {
		...
		//Constant argument
		$constant_arg = array( 'anchor' => $GLOBALS['WP_CLI_APP']['package']['wordpress_package_anchor'], 'placement' => 'after', 'raw' => true, 'normalize' => true );
