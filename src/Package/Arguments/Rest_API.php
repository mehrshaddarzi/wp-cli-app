...
class Rest_API {
	...
	public static function reset_rest_url_prefix() {
		...
		//Get Default Prefix
		$default_prefix = $GLOBALS['WP_CLI_APP']['package']['default_rest_prefix'];
	...
	public static function update_rest_api( $mu_plugin_path, $args ) {
		...
		if ( ! is_null( $args ) ) {
			...
			if ( is_bool( $args ) and $args === false ) {
				...
				//Change Prefix
				if ( isset( $args['prefix'] ) and is_string( $args['prefix'] ) and ! empty( $args['prefix'] ) and strtolower( $args['prefix'] ) != $GLOBALS['WP_CLI_APP']['package']['default_rest_prefix'] ) {
